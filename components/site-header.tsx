  ...
  return <header className="fixed inset-x-0 top-0 z-50 border-b border-copper/15 bg-night/90 backdrop-blur-xl">
    <nav className="mx-auto flex h-24 max-w-[1440px] items-center justify-between px-5 sm:px-10">
      ...
      <div className="flex items-center gap-2">
        ...
        <button className="grid size-12 place-items-center rounded-xl border border-copper/30 text-copper transition hover:bg-copper hover:text-night" onClick={toggleTheme} aria-label={theme === "dark" ? "Passer au thème clair" : "Passer au thème sombre"} title={theme === "dark" ? "Thème clair" : "Thème sombre"}>{theme === "dark" ? <Sun size={21} /> : <Moon size={21} />}</button>
